      ...
      try {
        ...
        if (categoryId == null) {
          ...
          response = await axios.get(
            `http://localhost:9090/api/product/fetch/seller-wise?sellerId=${seller.id}&status=Available`
          );
        } else {
          // Fetch products by category
          response = await axios.get(
            `http://localhost:9090/api/product/fetch/seller-wise/category-wise?sellerId=${seller.id}&categoryId=${categoryId}&status=Available`
          );
        }
        if (response.data) {
